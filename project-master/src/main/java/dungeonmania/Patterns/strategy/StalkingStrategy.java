...
public class StalkingStrategy<E extends Entity & MovingInterface> extends MovementStrategy<E>{

    private Map<Position, Position> result = new HashMap<>();
    public StalkingStrategy(Game g) {
        super(g);
    ...
    public Position getNextPosition() {
        this.reset();
        boolean found = this.flood(getPlayer().getPosition(), this.getEntity().getPosition());
        if (found && !this.result.isEmpty())
            return this.result.get(this.getEntity().getPosition());
        else if (found && this.result.isEmpty())
            return this.getEntity().getPosition();
        else
            return this.getClosestSquareToPlayer(this.getEntity().getPosition(), getPlayer().getPosition());
    }
